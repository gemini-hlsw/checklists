...
Checklists.Select2 = Ember.View.extend
  ...
  attributeBindings: ['placeholder', 'tabindex', 'multiple', 'containerCssClass', 'dropdownCssClass']
  multiple: false
  containerCssClass: ''
  dropdownCssClass: ''
  _data: ->
...
switchLink = (title, name, postfix) ->
  $("link[name=#{title}]").each ->
    this.href = "6/assets/stylesheets/#{title}-#{name}#{postfix}.css"
